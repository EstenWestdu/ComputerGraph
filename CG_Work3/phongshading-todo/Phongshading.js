...
function setFogDensity(d) {
	...
	if (disp) disp.textContent = fogDensity.toFixed(3);
}
...
function setFogMode(mode) {
	...
	var label = 'Linear';
	if (mode == 1) label = 'Exp';
	if (mode == 2) label = 'Exp2';
	var disp = document.getElementById('fogModeDisplay');
...
window.onload = function() {
	...
	// wire fog UI
	var btn = document.getElementById('fogCycleBtn');
	if (btn) {
		btn.addEventListener('click', function() {
			setFogMode((fogMode + 1) % 3);
			render();
		});
	}
	var inc = document.getElementById('fogIncBtn');
	var dec = document.getElementById('fogDecBtn');
	if (inc) inc.addEventListener('click', function(){ setFogDensity(fogDensity * 1.2); render(); });
	if (dec) dec.addEventListener('click', function(){ setFogDensity(fogDensity / 1.2); render(); });
	// keyboard shortcuts for density: [ to decrease, ] to increase
	window.addEventListener('keydown', function(e){
		if (e.key === '[') { setFogDensity(fogDensity / 1.2); render(); }
		if (e.key === ']') { setFogDensity(fogDensity * 1.2); render(); }
	});
	// keyboard F to cycle fog
	window.addEventListener('keydown', function(e){ if (e.key === 'f' || e.key === 'F') { setFogMode((fogMode+1)%3); render(); } });
	// initialize fog UI display according to current mode
	setFogMode(fogMode);
}
...
function render(){	
	...
	// density for exp/exp2 (use adjustable global)
	gl.uniform1f(gl.getUniformLocation(program, "fogDensity"), fogDensity);
	// fogMode: 0=linear,1=exp,2=exp2 -> use current fogMode global
